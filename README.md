...
### Use in-memory SQLite for development (no database setup required)
1. The app uses SQLite in-memory database by default, which doesn't require any server or file setup.
   - Data is stored in memory and resets on restart.
   - No `config.json` or `.env` needed for basic testing.
2. Run the app using the virtualenv:
   ```bash
...
- Switch to a persistent database like PostgreSQL by setting `DATABASE_URL`.
- For Railway: Add a PostgreSQL database, and Railway sets `DATABASE_URL` automatically.
- For local persistent SQLite: Set `DATABASE_URL=sqlite:///./vytrix.db` in `.env`.

### Notes
- The app reads environment variables first, then falls back to `config.json` or defaults.
